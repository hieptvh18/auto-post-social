...
# ── Cầu nối với backend NestJS (tool-auto-fb plan 28) ──────────────────────
#
# BA LỆNH DƯỚI ĐÂY DO MÁY GỌI, KHÔNG PHẢI NGƯỜI.
#
#   * stdout chỉ chứa ĐÚNG MỘT dòng JSON — mọi log người đọc đi ra stderr.
#     KHÔNG dùng `console.print` (rich) trong nhánh này: nó ghi vào stdout và
#     làm `JSON.parse` phía Node nổ (cạm bẫy C4 của plan 28).
#   * Lỗi ⇒ in JSON `{"ok": false, "errorCode": ...}` VÀ thoát mã != 0.
#   * API key đọc từ env `YOUTUBE_API_KEY`, không nhận qua tham số dòng lệnh
#     (argv hiện trong `ps`).


def _run_json_command(handler) -> None:
    """Khung chung: gọi handler, in JSON, map lỗi về tập mã đóng."""
    from .youtube import (
        ERROR_UNKNOWN,
        YoutubeError,
        emit,
    ...
    try:
        emit({"ok": True, **handler()})
    except YoutubeError as error:
        log(f"[error] {error.code}: {error.message}")
        emit_error(error.code, error.message)
        raise SystemExit(1)
    except Exception as error:  # noqa: BLE001 — biên ngoài cùng, không được để lọt traceback ra stdout
        log(f"[error] {type(error).__name__}: {error}")
        emit_error(ERROR_UNKNOWN, str(error))
        raise SystemExit(1)
...
@click.option("--region", default="VN", show_default=True)
@click.option("--published-after", "published_after_days", default=30, show_default=True)
@click.option("--json", "as_json", is_flag=True, default=True, help="Luôn bật — giữ cờ cho rõ ý ở phía gọi")
def yt_search(keyword, max_results, region, published_after_days, as_json):
    """Tìm video YouTube theo keyword. In JSON ra stdout, KHÔNG tải gì."""
    from .youtube import run_async, search_videos
    ...
    def handler():
        ...
        return {"videos": [video.to_json() for video in videos]}

    _run_json_command(handler)


@cli.command("yt-download")
@click.option("--url", required=True)
@click.option("--out", "out_dir", required=True, help="Thư mục TUYỆT ĐỐI, duy nhất theo job")
@click.option("--timeout", "timeout_sec", default=600, show_default=True)
@click.option("--max-filesize-mb", default=500, show_default=True)
@click.option("--json", "as_json", is_flag=True, default=True)
def yt_download(url, out_dir, timeout_sec, max_filesize_mb, as_json):
    """Tải 1 video YouTube về thư mục chỉ định. In JSON ra stdout."""
...
@cli.command("contract-version")
@click.option("--json", "as_json", is_flag=True, default=True)
def contract_version(as_json):
    """Cho backend kiểm tra downloader có chạy được không (checkAvailability)."""
    from .youtube import CONTRACT_VERSION, emit

    emit({"ok": True, "version": str(CONTRACT_VERSION)})
